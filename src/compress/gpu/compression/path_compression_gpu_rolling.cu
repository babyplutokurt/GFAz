#include "compress/gpu/compression/compression_workflow_gpu_internal.hpp"
#include "compress/gpu/core/codec_gpu.cuh"
#include "compress/gpu/compression/path_rulebook_gpu.hpp"
#include "compress/gpu/compression/path_compression_gpu_rolling.hpp"
...
gfaz::CompressedData compress_gpu_traversals_rolling_scheduler(
  ...
  for (int round_idx = 0; round_idx < num_rounds; ++round_idx) {
    ...
    result.layer_rule_ranges.push_back(
        gfaz::LayerRuleRange{2, next_start_id, next_start_id + num_rules_after_compact,
