#include "compress/gpu/compression/path_rulebook_gpu.hpp"
...
struct GreaterEqual {
  ...
  explicit GreaterEqual(uint32_t t) : threshold(t) {}
...
void filter_rules_by_count_device_vec(
  ...
  auto it = thrust::copy_if(d_unique_keys.begin(), d_unique_keys.end(),
                            d_total_counts.begin(), d_round_rules.begin(),
...
void or_reduce_rules_used_device_vec(
  ...
  thrust::transform(d_round_used.begin(), d_round_used.end(),
                    d_chunk_used.begin(), d_round_used.begin(),
...
void build_rule_compaction_map_device_vec(
  ...
  thrust::transform(
      d_rules_used_round.begin(), d_rules_used_round.end(), d_flags_int.begin(),
...
void build_rule_reorder_map_device_vec(
  ...
  thrust::sort_by_key(d_rules_copy.begin(), d_rules_copy.end(),
                      d_indices.begin());
