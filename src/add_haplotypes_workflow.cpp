      ...
      if (i > node_start) {
        ...
        if (!is_numeric(node_name)) {
          throw std::runtime_error(
              ...
              "CPU .gfaz does not store original segment names; appended P-lines "
              "must use numeric segment IDs");
        }
        path.push_back(parse_numeric_node(node_name, orientation,
                                          "P-line segment ID"));
      }
      node_start = i + 1;
    }
...
void parse_walk_line(std::string_view line, AppendInput &out) {
  ...
  while (walk_pos < walk_str.size()) {
    ...
    walk.push_back(parse_numeric_node(node_name, orientation,
                                      "W-line segment ID"));
    walk_pos = name_end;
  }

  out.walks.sample_ids.emplace_back(sample_id);
  out.walks.hap_indices.push_back(parse_uint32(hap_index, "W-line hap_index"));
  out.walks.seq_ids.emplace_back(seq_id);
  out.walks.seq_starts.push_back(
      parse_int64_or_star(seq_start, "W-line seq_start"));
  out.walks.seq_ends.push_back(parse_int64_or_star(seq_end, "W-line seq_end"));
...
AppendInput parse_append_file(const std::string &haplotypes_path) {
  ...
  while (std::getline(input, line)) {
    ...
    if (type == 'P') {
      if (parsed.has_walks) {
        throw std::runtime_error(std::string(kAddHaplotypesErrorPrefix) +
                                 "input file mixes P-lines and W-lines; use a pure path or pure walk file");
      }
...
std::vector<CompressionRules2Mer>
build_layered_rules(const CompressedData &data,
  ...
  for (const auto &range : data.layer_rule_ranges) {
    ...
    const size_t rule_offset = range.k == 0 ? 0 : (range.flattened_offset / range.k);
...
void encode_with_existing_rules(std::vector<std::vector<NodeId>> &sequences,
...
void validate_unique_names(const std::vector<std::string> &existing,
  ...
  for (const auto &name : incoming) {
    if (!seen.insert(name).second) {
      throw std::runtime_error(std::string(kAddHaplotypesErrorPrefix) + kind +
...
void append_paths(CompressedData &data, const AppendInput &input,
  ...
  apply_delta_and_validate(appended_paths, data.delta_round, data.min_rule_id(),
  ...
  existing_overlaps.insert(existing_overlaps.end(), input.path_overlaps.begin(),
...
void append_walks(CompressedData &data, const AppendInput &input,
  ...
  apply_delta_and_validate(appended_walks, data.delta_round, data.min_rule_id(),
                           !data.layer_rule_ranges.empty());
  ...
  data.walk_hap_indices_zstd = Codec::zstd_compress_uint32_vector(hap_indices);
