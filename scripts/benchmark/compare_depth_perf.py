#!/usr/bin/env python3
"""Benchmark `gfaz depth` against `odgi depth`.

odgi materializes the graph (`odgi build` then `odgi depth`); gfaz streams the
compressed `.gfaz` into a single shared per-node counter. Each command runs under
`/usr/bin/time -v` capturing wall time and peak RSS — the streaming RAM win is the
headline. Optionally checks value concordance of the per-node `-d` table (sorted
body); this is byte-exact only when the GFA segment names are 1..N (gfaz uses its
own node ids), so concordance may legitimately DIFFER on graphs with arbitrary
segment names even though the depth distribution is correct.

Examples:
  python scripts/benchmark/compare_depth_perf.py \
      --gfa /data/chrY.gfa --gfaz chrY.gfaz --threads 16 --concordance
"""
...
def main(argv):
  ...
  ap.add_argument("--mode", choices=["per-node", "summary"], default="per-node",
                  help="per-node == gfaz -d / odgi -d; summary == gfaz/odgi -S.")
  ...
  ap.add_argument("--concordance", action="store_true",
                  help="Compare sorted output bodies for value agreement.")
  args = ap.parse_args(argv)

  args.out_dir.mkdir(parents=True, exist_ok=True)
  gfaz_flag = {"per-node": ["-d"], "summary": ["-S"]}[args.mode]
  odgi_flag = {"per-node": ["-d"], "summary": ["-S"]}[args.mode]
  t = str(args.threads)
  ...
  gfaz_out = args.out_dir / "gfaz.depth.tsv"
  odgi_out = args.out_dir / "odgi.depth.tsv"
  og = args.out_dir / (args.gfa.stem + ".og")

  print(f"[gfaz depth {' '.join(gfaz_flag)}]")
  g_wall, g_rss, g_rc, g_el = timed(
      [str(args.gfaz_bin), "depth", "-i", str(gfaz), *gfaz_flag, "-t", t],
      gfaz_out)

  print("[odgi build]")
  b_wall, b_rss, b_rc, _ = timed(
      [str(args.odgi_bin), "build", "-g", str(args.gfa), "-o", str(og), "-t", t])
  print(f"[odgi depth {' '.join(odgi_flag)}]")
  o_wall, o_rss, o_rc, o_el = timed(
      [str(args.odgi_bin), "depth", "-i", str(og), *odgi_flag, "-t", t],
      odgi_out)
  ...
  print(f"odgi-depth\t{o_wall:.2f}\t{gib(o_rss)}\t{o_rc}")
  ...
  if args.concordance:
    ...
    print(f"\nconcordance (sorted body): {'MATCH' if same else 'DIFFER'} "
          f"(node ids coincide only when GFA segment names are 1..N)")
